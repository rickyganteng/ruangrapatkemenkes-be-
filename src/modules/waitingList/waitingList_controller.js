  ...
  postWaitingList: async (req, res) => {
    try {
      ...
      const checkNamaRuangBooking =
        await bookingRuanganModel.getDataConditionBooking(
          ruangYangDigunakan,
          ruangTanggalBooking,
          ruangWaktuMulai,
          ruangWaktuAkhir
      ...
      if (
        checkNamaRuangWaiting.length === 0 &&
        checkNamaRuangBooking.length === 0
      ) {
    ...
    try {
      ...
      if (result.length > 0) {
        ...
        if (checkNamaRuangWaiting.length === 0) {
          ...
          console.log('waiting', checkNamaRuangWaiting)
          console.log('waiting', checkNamaRuangBooking)
          console.log('waiting', checkNamaRuangWaiting.length)
          console.log('waiting', checkNamaRuangBooking.length)
          if (checkNamaRuangWaiting[0].id != id) {
            return helper.response(res, 400, 'Tanggal booking sudah terpakai')
          } else {
            result = await bookingRuanganModel.updateData(setData, id)
            return helper.response(res, 200, 'Succes Update Movie', result)
